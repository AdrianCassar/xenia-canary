...
def generate_version_h(build_dir="build"):
    ...
    if is_github_actions_environment():
        remote_details += f"""#define XE_BUILD_GITHUB_ACTIONS true\n"""
    else:
        remote_details += f"""#define XE_BUILD_GITHUB_ACTIONS false\n"""

    if remote_info.valid:
        remote_details += f"""#define XE_BUILD_REMOTE_OWNER "{remote_info.owner}"
#define XE_BUILD_REMOTE_NAME "{remote_info.repo}"
...
def git_get_head_info():
    ...
    (stdout, stderr) = p.communicate()
    remote_url = stdout.strip() or "unknown"
    remote_info = parse(remote_url)
    p = subprocess.Popen([
        "git",
        "rev-parse",
        "HEAD",
...
def is_github_actions_environment():
    """Returns true if we are inside Github Actions environment.
    """
