...
void UPnP::AddPort(std::string_view addr, uint16_t internal_port,
  ...
  auto run = [&]() {
    const std::string lease_duration = leases_supported_ ? "3600" : "0";  // 1h
    ...
    if (res == UPNPCOMMAND_SUCCESS) {
      ...
      if (update = (is_bound == port_bindings_[std::string(protocol)].end())) {
    ...
    port_binding_results_[std::string(protocol)][external_port] = res;
  };
...
void UPnP::RemovePort(uint16_t internal_port, std::string_view protocol) {
  ...
  if (&port_bindings_.at(str_protocol) ||
