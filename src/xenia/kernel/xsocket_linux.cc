...
#include "src/xenia/kernel/xsocket_linux.h"
...
int XSocket::PollWSARecvFrom(bool wait, WSARecvFromData receive_async_data) {
  ...
  uint32_t n_from = 0;
  uint32_t n_from_len = 0;

  uint32_t bytes_received = 0;
  uint32_t flags = receive_async_data.flags;
  auto buffers = new XWSABUF[receive_async_data.num_buffers];
  ...
  for (auto i = 0u; i < receive_async_data.num_buffers; i++) {
    ...
    buffers[i].buf_ptr = kernel_state()->memory()->TranslateVirtual(
  ...
  flags = 0;
  // if (msg.msg_flags & MSG_TRUNC) flags |= MSG_PARTIAL; // MSG_PARTIAL not on
  // linux?
...
void XSocket::SetLastWSAError(X_WSAError error) const { errno = (int)error; }
