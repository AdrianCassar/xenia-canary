...
X_HRESULT XLiveBaseApp::XUserFindUsers(uint32_t buffer_ptr) {
  ...
  FIND_USERS_RESPONSE* response_results_ptr =
  ...
  // Fixed 58410B5D
  memset(response_results_ptr, 0, internal_data_ptr->results_size);
  ...
  uint32_t user_info_size = num_users * sizeof(FIND_USER_INFO);

  const uint32_t find_user_info_address =
      kernel_state()->memory()->SystemHeapAlloc(user_info_size);

  FIND_USER_INFO* user_results_ptr =
      kernel_state()->memory()->TranslateVirtual<FIND_USER_INFO*>(
          find_user_info_address);
  ...
  response_results_ptr->results_size = user_info_size;
  response_results_ptr->users_address = find_user_info_address;
