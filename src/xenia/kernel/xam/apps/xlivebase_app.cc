...
X_HRESULT XLiveBaseApp::XStorageEnumerate(uint32_t buffer_ptr) {
  ...
  std::filesystem::path enumeration_path = std::filesystem::path(server_path);
  ...
  X_STORAGE_FACILITY facility_type =
      GetStorageFacilityTypeFromServerPath(enumeration_path.string());
  ...
  std::filesystem::path final_enumeration_path = enumeration_path;
  ...
  if (route_backend) {
    ...
    const auto enumeration_result =
        XLiveAPI::XStorageEnumerate(enumeration_path.string(), max_items);

    const auto& enumerated_files = enumeration_result.first;
    enumerated_backend = enumeration_result.second;

    std::filesystem::path item_parent = enumeration_path.parent_path();
    ...
    for (uint32_t item_index = starting_index;
      ...
      std::string filename = backend_path.filename().string();

      std::filesystem::path backend_item_path = std::format(
          "{}/{}", item_parent.string(), backend_path.filename().string());

      uint32_t item_path_size = static_cast<uint32_t>(
          backend_item_path.string().size() * sizeof(char16_t));
      ...
      xe::string_util::copy_and_swap_truncating(
          item_path_ptr, backend_item_path.u16string(), item_path_size);

      items_array_ptr[item_index].path_name =
          static_cast<uint32_t>(backend_item_path.string().size());
      ...
      items_array_ptr[item_index].ft_created =
          static_cast<uint32_t>(entry.Created());
      items_array_ptr[item_index].ft_last_modified =
          static_cast<uint32_t>(entry.LastModified());

      results_ptr->num_items_returned += 1;

      XELOGI("{}: Added storage item: {}", __func__,
             backend_path.filename().string());
  ...
  if (!route_backend || !enumerated_backend) {
    std::filesystem::path item_parent =
        ConvertServerPathToXStorageSymlink(enumeration_path.string());

    item_parent = item_parent.parent_path();

    final_enumeration_path = item_parent / enumeration_path.filename();
    final_enumeration_path = final_enumeration_path.make_preferred();

    // Match Wildcards: /*file.cfg and filename literal file.cfg
    std::string wildcard_item_filename = enumeration_path.filename().string();
    std::string item_filename_literal = enumeration_path.filename().string();
  ...
  XELOGI(
      ...
      max_results_to_return, final_enumeration_path.string());
...
X_HRESULT XLiveBaseApp::XStorageDelete(uint32_t buffer_ptr) {
  ...
  std::filesystem::path item_path = std::filesystem::path(server_path);
  ...
  X_STORAGE_FACILITY facility_type =
      GetStorageFacilityTypeFromServerPath(item_path.string());
  ...
  if (route_backend) {
    bool deleted = XLiveAPI::XStorageDelete(item_path.string());
    result = deleted ? X_E_SUCCESS : X_E_FAIL;
  }

  if (!route_backend || result) {
    item_path = ConvertServerPathToXStorageSymlink(item_path.string());

    vfs::Entry* storage_item =
        kernel_state()->file_system()->ResolvePath(item_path.string());
  ...
  XELOGI("{}: {}", __func__, item_path.string());
...
X_HRESULT XLiveBaseApp::XStorageDownloadToMemory(uint32_t buffer_ptr) {
  ...
  std::filesystem::path item_to_download = std::filesystem::path(server_path);

  X_STATUS result = X_E_FAIL;

  X_STORAGE_FACILITY facility_type =
      GetStorageFacilityTypeFromServerPath(item_to_download.string());
  ...
  if (route_backend) {
    std::span<uint8_t> buffer =
        XLiveAPI::XStorageDownload(item_to_download.string());
  ...
  if (!route_backend || result) {
    std::filesystem::path filename = item_to_download.filename();
    item_to_download =
        ConvertServerPathToXStorageSymlink(item_to_download.string());

    xe::vfs::File* output_file;
    xe::vfs::FileAction action = {};

    X_STATUS open_result = kernel_state_->file_system()->OpenFile(
        nullptr, item_to_download.string(), xe::vfs::FileDisposition::kOpen,
    ...
    if (!open_result) {
      ...
      if (!open_result) {
        ...
        XELOGI("{}: Downloaded: {}", __func__, filename.string());
        result = X_E_SUCCESS;
      } else {
        XELOGI("{}: Failed to download: {}", __func__, filename.string());
        result = X_E_FAIL;
      }
    } else {
      XELOGI("{}: {} doesn't exists!", __func__, filename.string());
...
X_HRESULT XLiveBaseApp::XStorageUploadFromMemory(uint32_t buffer_ptr) {
  ...
  std::filesystem::path upload_file_path = server_path;
  std::filesystem::path filename = upload_file_path.filename();

  X_STATUS result = X_E_FAIL;

  X_STORAGE_FACILITY facility_type =
      GetStorageFacilityTypeFromServerPath(upload_file_path.string());
  ...
  if (route_backend) {
    bool uploaded =
        XLiveAPI::XStorageUpload(upload_file_path.string(), upload_buffer);

    result = uploaded ? X_E_SUCCESS : X_E_FAIL;
  }

  if (!route_backend || result) {
    upload_file_path =
        ConvertServerPathToXStorageSymlink(upload_file_path.string());

    // Check if entry exists
    vfs::Entry* entry = kernel_state()->file_system()->ResolvePath(
        upload_file_path.parent_path().string());
    ...
    if (entry) {
      ...
      result = kernel_state()->file_system()->OpenFile(
          nullptr, upload_file_path.string(),
          vfs::FileDisposition::kOverwriteIf, vfs::FileAccess::kGenericWrite,
          false, true, &upload_file, &action);
  ...
  if (!result) {
    XELOGI("{}: Uploaded {}", __func__, filename.string());
  } else {
    XELOGI("{}: Uploading {} failed with error {:08X}", __func__,
