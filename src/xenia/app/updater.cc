...
bool Updater::UpdateAndRestart(const std::filesystem::path& zip_path) {
  ...
  std::string script_content = fmt::format(
      ...
      "cd \"{1}\"\n"
      "echo [%date% %time%] Waiting for Xenia process to exit >> %LOG_FILE%\n"
      ":loop\n"
      ...
      "  goto loop\n"
      ")\n"
      "echo [%date% %time%] Xenia process has exited >> %LOG_FILE%\n"
      "echo [%date% %time%] Cleaning and creating backup folder >> %LOG_FILE%\n"
      "if exist \"{1}\\old\" rd /s /q \"{1}\\old\"\n"
      "mkdir     \"{1}\\old\"\n"
      "echo [%date% %time%] Backing up old executable: {2} >> %LOG_FILE%\n"
      "copy /y \"{2}\" \"{1}\\old\\{0}\" >> %LOG_FILE% 2>&1\n"
      "echo [%date% %time%] Extracting Xenia update... >> %LOG_FILE%\n"
      "echo [%date% %time%] Attempting tar extraction of {3} >> %LOG_FILE%\n"
      "tar -xf {3} >> %LOG_FILE% 2>&1\n"
      "if errorlevel 1 (\n"
      "  echo [%date% %time%] tar extraction failed, trying PowerShell >> "
      "%LOG_FILE%\n"
      "  powershell -ExecutionPolicy RemoteSigned -Command \"$ErrorActionPreference "
      ...
      "}}\" >> %LOG_FILE% 2>&1\n"
      "  if errorlevel 1 (\n"
      "    echo [%date% %time%] ERROR: Both tar and PowerShell extraction "
      "failed >> %LOG_FILE%\n"
      "    echo Please extract the update manually: {4}\n"
      "    echo [%date% %time%] Creating manual update instructions file >> "
      "%LOG_FILE%\n"
      "    echo MANUAL UPDATE REQUIRED > \"{1}\\MANUAL_UPDATE_REQUIRED.txt\"\n"
      "    echo. >> \"{1}\\MANUAL_UPDATE_REQUIRED.txt\"\n"
      "    echo Automatic extraction failed for both tar and PowerShell. >> "
      "\"{1}\\MANUAL_UPDATE_REQUIRED.txt\"\n"
      "    echo. >> \"{1}\\MANUAL_UPDATE_REQUIRED.txt\"\n"
      "    echo To update Xenia manually: >> "
      "\"{1}\\MANUAL_UPDATE_REQUIRED.txt\"\n"
      "    echo 1. Extract the zip file: {4} >> "
      "\"{1}\\MANUAL_UPDATE_REQUIRED.txt\"\n"
      "    echo 2. Replace the old Xenia executable with the new one >> "
      "\"{1}\\MANUAL_UPDATE_REQUIRED.txt\"\n"
      "    echo 3. Delete this file and the zip file when done >> "
      "\"{1}\\MANUAL_UPDATE_REQUIRED.txt\"\n"
      "    echo. >> \"{1}\\MANUAL_UPDATE_REQUIRED.txt\"\n"
      "    echo Zip file location: {4} >> \"{1}\\MANUAL_UPDATE_REQUIRED.txt\"\n"
      "    echo Backup of old executable: {1}\\old\\{0} >> "
      "\"{1}\\MANUAL_UPDATE_REQUIRED.txt\"\n"
      "    echo [%date% %time%] Update failed - user intervention required >> "
      "%LOG_FILE%\n"
      "    start \"\" \"{1}\\MANUAL_UPDATE_REQUIRED.txt\"\n"
      "    del \"%~f0\"\n"
      "    exit /b 1\n"
      "  ) else (\n"
      "    echo [%date% %time%] PowerShell extraction succeeded >> %LOG_FILE%\n"
      "  )\n"
      ") else (\n"
